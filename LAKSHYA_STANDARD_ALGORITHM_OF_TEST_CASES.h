...
namespace lakshya_standard_algorithm_of_test_cases
{
    ...
    public:
        ...
        if(sizee<prime.size())
        {
            randomized_vector(prime);
            while(prime.size()!=sizee) prime.pop_back();
        }
        ...
        vector <ll> get_fibonacci(ll lower_limit,ll upper_limit,ll sizee)
        {
            ...
            if(sizee<fibonacci.size())
            {
                randomized_vector(fibonacci);
                while(fibonacci.size()!=sizee) fibonacci.pop_back();
            }
